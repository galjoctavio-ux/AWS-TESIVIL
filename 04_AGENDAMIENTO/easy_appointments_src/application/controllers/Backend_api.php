    ...
    public function ajax_save_appointment()
    {
        try
        {
            ...
            if ($this->input->post('appointment_data'))
            {
                ...
                // --- CUSTOM TESIVIL: Asegurar tipos de datos custom ---
                // Forzamos que el buffer sea un entero (0 si no viene definido)
                $appointment['buffer_traslado'] = isset($appointment['buffer_traslado']) ? (int)$appointment['buffer_traslado'] : 0;
                // ¡AQUÍ ESTÁ EL ARREGLO!
 // Forzamos que el link sea un texto (o nulo si viene vacío)
    $appointment['direccion_link'] = isset($appointment['direccion_link']) ? (string)$appointment['direccion_link'] : NULL;
// -----------------------------------------------------
                
                // --- CUSTOM TESIVIL: Guardar datos custom ANTES de recargar ---
                // Guardamos los valores aquí, porque get_row() los va a borrar.
                $custom_location = isset($appointment['location']) ? $appointment['location'] : NULL;
                $custom_link = isset($appointment['direccion_link']) ? $appointment['direccion_link'] : NULL;
                $custom_buffer = $appointment['buffer_traslado'];
                // -----------------------------------------------------
            ...
            // --- CUSTOM TESIVIL: Reinyección de datos custom ---
            // Sobrescribimos el array $appointment con los datos que se perdieron en get_row()
            $appointment['location'] = $custom_location;
            $appointment['direccion_link'] = $custom_link;
            $appointment['buffer_traslado'] = $custom_buffer;
            // ---------------------------------------------------
            ...
            error_log('DEBUG SYNC DATA: ' . print_r($appointment, true));
