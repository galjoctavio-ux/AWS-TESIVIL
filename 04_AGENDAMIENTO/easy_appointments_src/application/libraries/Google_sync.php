...
class Google_sync {
    ...
    public function add_appointment($appointment, $provider, $service, $customer, $settings)
    {
        ...
        // --- CUSTOM TESIVIL: Usar URL de Google Maps si existe ---
        if (!empty($appointment['direccion_link'])) {
            $event->setLocation($appointment['direccion_link']);
        } else if (!empty($appointment['location'])) {
            $event->setLocation($appointment['location']);
        } else {
            $event->setLocation($settings['company_name']);
        }
        // --------------------------------------------------------

        // --- CUSTOM TESIVIL: Lógica de Ubicación y Magic Link (AÑADIR) ---
        $full_description = $appointment['notes'];

        // Añadir dirección legible
        if (!empty($appointment['location'])) {
            $full_description .= "\n\nDirección: " . $appointment['location'];
        }

        // ¡NUEVA LÓGICA DE MAGIC LINK!
        if (!empty($appointment['notas_estructuradas'])) {
            try {
                $notas_json = json_decode($appointment['notas_estructuradas'], TRUE);

                if (is_array($notas_json) && isset($notas_json['magic_token'])) {
                    $magic_token = $notas_json['magic_token'];
                    $magic_link = 'https://www.tesivil.com/lete/app/revision?token=' . $magic_token;

                    $full_description .= "\n\n---------------------------------";
                    $full_description .= "\nFormulario de Revisión (Técnico):\n" . $magic_link;
                }
            } catch (Exception $e) {
                // Si el JSON está malformado, simplemente no añadimos el link
            }
        }

        $event->setDescription($full_description);
        // ---------------------------------------------------
        ...
        // --- CUSTOM TESIVIL: Aplicar Buffer de Traslado al tiempo final ---
        $end_datetime_obj = new DateTime($appointment['end_datetime'], $timezone);
        if (!empty($appointment['buffer_traslado']) && (int)$appointment['buffer_traslado'] > 0) {
            // Si hay buffer, añadimos esos minutos a la hora de fin que se envía a Google.
            $end_datetime_obj->modify('+' . (int)$appointment['buffer_traslado'] . ' minutes');
        }

        $end = new Google_Service_Calendar_EventDateTime();
        $end->setDateTime($end_datetime_obj->format(DateTime::RFC3339));
        $event->setEnd($end);
        // ----------------------------------------------------------------
    ...
    public function update_appointment($appointment, $provider, $service, $customer, $settings)
    {
        ...
        // --- CUSTOM TESIVIL: Lógica de ubicación y notas (UPDATE) ---
        if (!empty($appointment['direccion_link'])) {
            $event->setLocation($appointment['direccion_link']);
        } else if (!empty($appointment['location'])) { 
            $event->setLocation($appointment['location']);
        ...
        // --- CUSTOM TESIVIL: Lógica de Ubicación y Magic Link (ACTUALIZAR) ---
        $full_description = $appointment['notes'];
        ...
        // --- CUSTOM TESIVIL: Aplicar Buffer de Traslado (UPDATE) ---
        $end_datetime_obj = new DateTime($appointment['end_datetime'], $timezone);
        if (!empty($appointment['buffer_traslado']) && (int)$appointment['buffer_traslado'] > 0) {
            $end_datetime_obj->modify('+' . (int)$appointment['buffer_traslado'] . ' minutes');
        }
