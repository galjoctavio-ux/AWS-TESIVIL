...
const stripe = new Stripe(process.env.STRIPE_SECRET_KEY!, {
  apiVersion: '2025-09-30.clover', // Versión de API corregida
});
...
async function saveOrderToSupabase(session: Stripe.Checkout.Session) {
  ...
  const customerRfc = session.customer_details?.tax_ids?.[0]?.value || null;
  
  // --- CORRECCIÓN DE FUERZA BRUTA (as any) ---
  let shippingAddress = 'Dirección no proporcionada'; // Valor por defecto
  // Le decimos a TypeScript que confíe en nosotros con (session as any)
  if ((session as any).shipping_details && (session as any).shipping_details.address) {
    const { line1, line2, city, state, postal_code, country } = (session as any).shipping_details.address;
    shippingAddress = `${line1}${line2 ? ', ' + line2 : ''}, ${city}, ${state} ${postal_code}, ${country}`;
  }
  ...
  const orderData = {
    ...
    terms_accepted_version: session.metadata?.terms_accepted || null,
...
async function sendLogisticsEmail(session: Stripe.Checkout.Session, orderId: number) {
  ...
  // --- CORRECCIÓN DE FUERZA BRUTA (as any) ---
  let shippingAddress = 'Dirección no proporcionada';
  // Repetimos la misma corrección aquí
  if ((session as any).shipping_details && (session as any).shipping_details.address) {
    const { line1, line2, city, state, postal_code, country } = (session as any).shipping_details.address;
    shippingAddress = `${line1}${line2 ? ', ' + line2 : ''}, ${city}, ${state} ${postal_code}, ${country}`;
  }
  ...
  try {
    await resend.emails.send({
      from: 'Ventas Cuentatrón <ventas@tu-dominio.com>', // REEMPLAZA ESTO
      to: logisticsEmail,
...
export async function POST(req: Request) {
  ...
  try {
    ...
    console.error('Webhook: Error al procesar el evento:', err.process.message);
