 ...
 */
const sendPushNotification = async (userId: string, title: string, body: string, data?: { [key: string]: string }) => {
    try {
 ...
 */
export const onSOSReply = functions.firestore
    ...
    .onCreate(async (snap, context) => {
        ...
        try {
            ...
            await sendPushNotification(
                ...
                `${comment.author_alias || 'Alguien'} respondió en tu hilo: ${thread?.title || 'SOS'}`,
                { type: 'sos_reply', threadId }
            );
 ...
 */
export const onSolutionAccepted = functions.firestore
    ...
    .onUpdate(async (change, context) => {
        ...
        if (!before.solution_accepted_comment_id && after.solution_accepted_comment_id) {
            ...
            try {
                ...
                const solverId = comment?.author_id;

                if (!solverId) return;

                await sendPushNotification(
                    solverId,
                    '🎉 ¡Tu solución fue aceptada!',
                    `Tu respuesta ayudó a resolver un problema. ¡Ganaste Tokens!`,
                    { type: 'sos_reply', threadId }
                );
    ...
    .onRun(async () => {
        ...
        try {
            ...
            for (const doc of snapshot.docs) {
                ...
                // Check Preferences
                if (userData.notificationPreferences?.maintenanceReminders === false) {
                    continue;
                }
                ...
                await sendPushNotification(
                    userId,
                    '🔧 Mantenimiento Sugerido (7 días)',
                    `El equipo de ${clientName} requiere servicio pronto.`,
                    { type: 'maintenance_reminder', serviceId: doc.id, clientId: service.client_id }
                );
            }
