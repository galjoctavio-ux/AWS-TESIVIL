...
export const checkReminders = async () => {
    ...
    try {
        ...
        // A) Citas para MAÑANA
        // Definición: La fecha guardada (UTC) convertida a MX debe ser igual a (NOW en MX + 1 día)
        const tomorrowClients = await query(`
            ...
            AND whatsapp_id NOT LIKE '%@g.us' -- Filtro de seguridad (Grupos)
        `);
        ...
        // Definición: La fecha guardada (UTC) convertida a MX debe ser igual a (NOW en MX)
        ...
        // Buscamos tareas cuya fecha programada YA LLEGÓ (<= NOW())
        ...
        for (const task of followUps.rows) {
            ...
            // Antes de enviar, verificamos si un humano (o el bot en otro hilo) ya contestó recientemente.
            ...
            if (lastMsgCheck.rows.length > 0) {
                ...
                // Si el último mensaje es NUESTRO (Internal = true), abortamos.
                // Significa que ya retomamos la conversación manualmente.
                if (lastMsg.is_internal) {
                    console.log(`✋ Cancelando envío auto a ${task.whatsapp_id}: Humano ya intervino.`);

                    // Cancelamos la tarea para que no se repita
            ...
            // 1. GHOSTING CLÁSICO (Sin respuesta inicial)
            ...
            // Enviamos el mensaje si se definió alguno
            if (message) {
                ...
                if (sentId) {
                    await query(`
                        ...
                        SET follow_up_status = 'SENT',
                            intent = 'AWAITING_REPLY', -- Cambiamos intención a esperar respuesta (evita re-análisis inmediato)
